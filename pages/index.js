...
export default function Home() {
  ...
  useEffect(() => {
    ...
    console.log("auth token is exist: ", authToken)
    if (authToken) {
      localStorage.setItem("auth_token", authToken);
      const parsed = parseJwt(authToken);
      console.log("token parse result: ",parsed, parsed.user_id)
  ...
  const joinRoom = () => {
    if (roomId && username) {
      ...
      setUsername(`User_${myId?.slice(0, 8)}`)
    }
    else alert("Please provide a valid room id and username");
  };
